...
export async function POST(req: NextRequest) {
  ...
  const BASE = `*{margin:0;padding:0;box-sizing:border-box}body{background:#05070c;font-family:Arial,Helvetica,sans-serif;color:#e5e7eb}.wrap{background:#05070c;padding:40px 20px}.container{max-width:600px;margin:0 auto;background:linear-gradient(180deg,#05070c,#070b12);border:1px solid rgba(255,255,255,.14)}.header{padding:24px 20px;border-bottom:1px solid rgba(255,255,255,.08);text-align:center}.hero{padding:36px 40px 28px;border-bottom:1px solid rgba(255,255,255,.06)}.eyebrow{font-size:11px;font-weight:500;letter-spacing:.2em;text-transform:uppercase;color:#00A651;margin-bottom:10px}.hero-title{font-size:28px;font-weight:700;color:#fff;line-height:1.2;margin-bottom:12px}.hero-sub{font-size:15px;color:#9ca3af;line-height:1.7}.block{padding:32px 40px;border-bottom:1px solid rgba(255,255,255,.06)}.block-label{font-size:10px;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:#00A651;margin-bottom:14px}.block-title{font-size:22px;font-weight:700;color:#fff;margin-bottom:10px}.block-body{font-size:14px;color:#9ca3af;line-height:1.8;margin-bottom:20px}.block-body strong{color:#fff}.perks{background:rgba(0,166,81,.06);border:1px solid rgba(0,166,81,.2);border-radius:8px;padding:18px 20px;margin-bottom:22px}.perk{font-size:13px;color:#9ca3af;padding:6px 0;border-bottom:1px solid rgba(255,255,255,.05)}.perk:last-child{border-bottom:none}.perk strong{color:#00A651}.discount-box{background:rgba(0,166,81,.08);border:1px solid rgba(0,166,81,.25);border-radius:8px;padding:20px;margin-bottom:22px;text-align:center}.discount-big{font-size:42px;font-weight:900;color:#00A651;line-height:1}.discount-label{font-size:12px;color:#9ca3af;margin-top:4px}.sig{padding:28px 40px;border-bottom:1px solid rgba(255,255,255,.06)}.sig-name{font-size:15px;font-weight:600;color:#fff;font-style:italic}.sig-title{font-size:11px;color:#00A651;letter-spacing:.15em;text-transform:uppercase;margin-top:4px}.footer{padding:18px 40px;text-align:center}.footer-text{font-size:11px;color:#374151;line-height:1.8}.footer-text a{color:#4b5563;text-decoration:underline}`

  function buildHtml(name: string, unsubToken: string): string {
    const unsub = `https://tee365.org/api/unsubscribe?token=${unsubToken}`
    return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Tee365</title><style>${BASE}</style></head><body>
<div class="wrap"><table class="container" width="100%" cellpadding="0" cellspacing="0" role="presentation">
  ...
  for (const r of recipients) {
    const name = r.first_name?.trim() || "there"
    const html = buildHtml(name, r.unsubscribe_token ?? "")
    try {
      ...
      if (res.ok) {
        ...
        results.failed++
        results.errors.push(`${r.email}: ${res.status}`)
      }
    } catch (e) {
      results.failed++
      results.errors.push(`${r.email}: ${String(e)}`)
    }
    // Small delay to avoid rate limiting
    await new Promise(r => setTimeout(r, 100))
  }

  // Mark all successfully sent as promo_code_sent
  if (sentEmails.length) {
    await supabase
      .from("waitlist")
      .update({ promo_code_sent: true, promo_code_sent_at: new Date().toISOString() })
      .in("email", sentEmails)
  }
