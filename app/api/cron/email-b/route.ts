...
async function sendOne(email: string, name: string, unsubToken: string): Promise<boolean> {
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: [email],
      subject: SUBJECT,
      html: buildHtml(name, unsubToken),
    }),
  })
  return res.ok
}
...
export async function GET(request: NextRequest) {
  ...
  // Test mode: ?test=1 sends only to Jerrod, skips campaign_sends entirely
  ...
  if (isTest) {
    ...
    const ok = await sendOne(me.email, me.first_name?.trim() || "Jerrod", me.unsubscribe_token ?? "")
  ...
  // Insert into campaign_sends BEFORE sending — unique constraint prevents double-send
  ...
  return NextResponse.json({ sent: toSend.length, skipped: all.length - toSend.length })
}
