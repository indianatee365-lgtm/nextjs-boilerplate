...
// Navigation renders as a plain span in the read-only admin mirror so the
// admin can't click through into the customer's context.
function Nav({
  interactive,
  href,
  className,
...
// The single source of truth for what a customer sees on /account.
//
// Two callers render this: the customer's own /account page (interactive), and
// the admin "View as customer" mirror at /admin/users/[id]/dashboard
// (interactive={false}, wrapped inert by that page). Both were hand-maintained
// copies until 2026-09-14 and had drifted - the mirror was missing My shots,
// hour credits, the giveaway redemption box, membership reinstatement, and any
// membership that wasn't status=active. Add new account sections here only, so
// the two can't diverge again.
//
// interactive={false} downgrades every navigation and the sign-out form to
// inert markup. That is deliberate belt-and-braces: the inert wrapper alone
// would block the clicks, but the section components below fire server actions
// against the *session* user, so an admin viewing a customer must not be one
// failed inert polyfill away from cancelling their own membership.
    ...
    <main className="mx-auto max-w-3xl px-4 py-10">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {profile?.role === "admin" && (
            <Nav interactive={interactive} href="/admin" className="btn-secondary text-xs px-3 py-2">Admin</Nav>
          )}
          {interactive ? (
            <form action={logout}>
            ...
            <span className="text-xs px-3 py-2 rounded-lg border border-red-500/50 text-red-400">Sign out</span>
          )}
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Nav interactive={interactive} href="/book" className="btn-primary flex items-center justify-center py-3">
          Book a bay
        </Nav>
        <Nav interactive={interactive} href="/account/bookings" className="btn-secondary flex items-center justify-center py-3">
          My bookings
        </Nav>
        <Nav interactive={interactive} href="/account/shots" className="btn-secondary flex items-center justify-center py-3">
          My shots
        </Nav>
        <Nav interactive={interactive} href="/gift-cards" className="btn-secondary flex items-center justify-center py-3">
          Gift cards
        </Nav>
      </div>
