...
- [ ] **🚨 LAUNCH BLOCKER — Vestibule + access control + on-site network**
  ...
  - **Ordering sequence (bench-first to limit risk):**
    ...
    - [ ] **Phase 2 — cameras + production door hardware + cabling materials (~$2,200).** Target: 2026-06-22
    - [ ] **Glazier contract signed + door prep spec delivered.** Target: 2026-06-22
    - [ ] **UDM-Pro installed at site; ISP active.** Target: 2026-06-22
    - [ ] **Vestibule installed** (4–8 wk glazier lead time). Target: 2026-07-20
    - [ ] **Access Ultra wired to strike + REX + DPS** (DIY or low-voltage installer). Target: 2026-07-25
    - [ ] **Wire `lib/access-control/index.ts` to Unifi API** + DB migration (`unifi_visitor_id`) + observability hooks. Target: 2026-07-28
    - [ ] **End-to-end live test:** booking → SMS → keypad → door unlocks. Target: 2026-08-01
  - **Access code:** 6 digits, generated by Unifi controller (not by Tee365). Existing randomInt PIN logic gets replaced.
  - **Ongoing operating cost:** **$0/mo** (no Unifi subscription, no per-door fee). Optional ~$30/mo cellular WAN failover.
